...
**1,433 tests across 70 files** (the Phase 4 gate of the IT-OS experience — the Ghid TBS guide,
2026-09-17; the result of each full run is recorded in [`CHANGELOG.md`](../CHANGELOG.md), and the
per-file counts below are from that run). Phase 4 added `lib/__tests__/guide-linger.test.ts` (75)
...
clean console and CSP, a light service page, the guide away over `#estimare` and the phone footer). Before the interior redesign: 206 in 13 — it added
...
- **The HUD is off unless a spec opts in.** Every context starts with `localStorage.tbs_hud =
  "off"` (`playwright.config.ts`), so no guide can arm mid-spec over the control a test presses. The
  HUD specs use `test.use({ storageState: HUD_ON })`, seed consent and call `armHud(page)` (a pointer
  move repeated every 250ms until a `[data-hud]` part is attached, 5s at most). `guideRoot`,
  `guideAvatar` and `guideTip` locate the guide; its copy is imported from
  `components/hud/guide/copy.ts`. The linger tip is waited for in real time (absent at 3.5s, present by
...
- **The survey run, once per phase:** `E2E_HUD=on npx playwright test` drops the seeded `tbs_hud=off`,
  ...
  passed, 0 failed — nothing to triage.
