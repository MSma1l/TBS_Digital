...
**1,613 tests across 75 files** (the Phase 5 gate of the IT-OS experience — the fibre scroll rail,
...
`scene-helix.test.ts` (41), `scene-helix-model.test.ts` (29), `scene-hologram.test.ts` (23) and
and
...
| Intro contract | `lib/__tests__/intro.test.ts` (27) · `intro.server.test.ts` (4) | the skip cookie (only the literal `seen` counts, read by its exact name, and it does **not** answer to the name it used to have); `finishIntro` writing only the legacy clear and never marking the intro as seen, its idempotence, `isIntroPending`, `onIntroDone`'s synchronous path, `shouldPlayIntro`; `tbs:intro-gone` (`markIntroGone` a no-op while the overlay is attached, fired once; `onIntroGone` synchronous without an overlay); importing `lib/intro` touches no DOM |
...
| Intro math | `intro-capability.test.ts` (26) · `intro-math.test.ts` (18) · `intro-scene-math.test.ts` (23) | the capability probe (jsdom, reduced motion, Save-Data, caveat, software-renderer detection, force 3D; the session cache: one probe context per tab, yes/no values only, no renderer name, never read when a live gate said no); tiers and the DPR clamp (`TIER_CONFIG` never costing more on a lower tier, its two extra draw calls spent only at the top and in order, and carrying no per-tier geometry counts — those belong beside the drop table in `three/laptop.ts`); `createIntroFx`; seeded orbits; `swayWeight` (exactly zero while the camera is inside the machine, full where the machine is seen whole at K4/K5, gone again before the display covers the frame, no corners, peak pan under three degrees with the pointer included); the FPS governor, including a steady 30 Hz cap that must not step down |
| Intro flight | `intro-camera-path.test.ts` (28) · `intro-laptop.test.ts` (19) | The shot list, import-free and pure: eight keys tiling `[0, 1]` strictly ascending, K2 the cross-fade's landing frame at u 0.40, every key inside the 0.01 / 14 clip budget at every aspect, the widen cap, no degenerate fov; `cameraAt` returning each key's own pose, continuous either side of every seam, clamped at both ends, NaN-proof, not assuming `u` climbs (a skip that jumps lands where it would have arrived) and never cutting — no 1% of the flight moves further than a tenth of the view depth. Then the flight **against the machine**: inside the chassis and off the die all the way to the vent — the window is 0.59, not 0.40, because K3 pushed the exit from `u` ~0.45 to 0.5950 (the widen cap) — 0.6058 (16:10), and the old window stopped at K2 and so checked nothing about the corridor the test exists to guard; the HATCH in shot at K2 — the way out is in the keyboard now, because that is where the processor is — leaving up through it exactly ONCE at every aspect (a path that popped out and dipped back in would satisfy an aperture check and still be nonsense), never getting behind the lid at the angle the lid actually has at that moment, and staying clear of the deck; the display 16:10 and covering the viewport at every aspect, never letter-boxed, with the table's K6 matching the derived plane. `flightFromProgress` 0 → 0.84, monotone, landing on every `FLIGHT_MAP` boundary and giving each beat a share; `lidOpenAt` shut until 0.40 and settled by 0.60, a couple of degrees past the top (mass, not a bounce) — **before** the camera surfaces, because a shut lid lies flat 0.02 above the deck's top and a closed laptop has no way out of the top; `screenFillAt` starting inside the lid's window and two thirds drawn at the exit, so there is something awake to surface into. The machine itself: 41 slots, every index used once, tiling with the named groups, the port given up last and the FRONT key row next to last — the two rows that frame the hatch survive the lite step by name, because the camera comes out between them — and, named one by one, **all twelve guts surviving the lite step**, because they are what the camera spends beat 3 threading between and the governor must never be allowed to take them, every box real and inside the machine's reach; `aU` climbing strictly and reaching the geometry as one instanced float per slot; the display wiped bottom to top and the board lit from the die outwards, finishing before the cross-fade; the lid shut at u 0 and open to 107° at u 1, carrying the display and the cover glass; six draw calls on high and four below, exactly one transmissive surface and only on high, `setLite` dropping pieces and never materials, each tier starting on its own share of the table; and, sampled across aspects, no piece ever inside the near plane and the camera inside the cavity until it is in the hatch — the one place it is allowed to pass the ceiling. Finally **the three settings that fail silently *and in the picture*** — the frame material is `DoubleSide` (with `FrontSide` the cavity has no inside and K2 is three hinge barrels in black), the cover pane never writes depth (it is the only depth writer in an additive scene, and a pane that writes it blacks out the display for beats 4–5), and the die's plate and the board's tracks are written one `uFill` and one `uHead` with the plate the darker of the two (two materials, one wave). Reverting any of them still compiles, still draws, still costs the same six draw calls and still passes every other test here. Nothing tuned by eye is pinned — not the edge/face/fresnel levels, not the three gains, not K4's aim: a threshold on an aesthetic number is a test that moves with the eye |
...
| The holographic portrait | `sections.test.tsx` | The card's photograph is projected, not replaced: three copies of the same `<img>` (him, the torn band, the beam), the `alt` on the first and only the first, the other two `aria-hidden` and empty; the key and the tint defined ONCE per section rather than per card, in sRGB because the thresholds were measured there, with a transfer that holds to 0.90 and falls to 0 at 1.0 — his jacket peaks at 0.904 and the ground reads exactly 1.000; and no projection invented for a member who has no photograph |
| The principles' card marks | `sections.test.tsx` | All three cards carry a mark and they are the SAME mark — same viewBox, same stroke, same fill — with three different drawings inside that identical box, because the row only reads as a row when the three match; each is silent (`aria-hidden`, `focusable="false"`, no text, no `<title>`); none draws a circle or a round cap, the house rule for line art; and nothing in the section shows a reading — the only digits allowed are its own card numbers, and `50+`, `98%` and `24/7` stay out of the markup AND out of `lib/content.ts`, whose dead `statusBars` export held them until this change |
...
- **Every spec skips the intro unless it says otherwise.** `gotoHydrated(page, url)` seeds
  `tbs_intro_skip=seen` (`INTRO_COOKIE`, straight out of `lib/intro`) before navigating, so no
  overlay sits over the controls a test is about to press. The site itself never writes that
  cookie — seeding it is the *only* way to get a page on `/` without an overlay. `e2e/preloader.spec.ts` passes `{ seedIntro: false }` for a real first visit;
