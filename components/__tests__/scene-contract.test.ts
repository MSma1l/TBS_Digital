 ...
 *   2. none of ScrollTrigger's page-rewriting features under components/scene/**: no pinning
 *      (spacers shift every anchor), no snapping (inline scroll-behavior on html/body), no
 *      normalizeScroll / ScrollSmoother, no markers, no custom scroller, no lagSmoothing (the
...
describe("interior stage — ScrollTrigger stays harmless", () => {
  const sceneFiles = listFiles("components/scene", CODE);

  it("scans the stage's own files", () => {
  ...
  it("no file under components/scene uses a forbidden feature", () => {
